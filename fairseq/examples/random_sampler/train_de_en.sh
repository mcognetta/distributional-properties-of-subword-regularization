...
EXPERIMENT_NAME="${EXPERIMENT_PREFIX}_VOCAB_${SRC_BPE_TOKENS}_${TGT_BPE_TOKENS}_type_${TOKENIZER_TYPE}_dropout_${SRC_DROPOUT}_${TGT_DROPOUT}_seed_${SEED}.${lang}"
...
if [ -d "../../de_en_experiments/${EXPERIMENT_NAME}" ]
then
...
mkdir -p de_en_experiments/${EXPERIMENT_NAME}/
...
CUDA_VISIBLE_DEVICES=$DEVICE nohup fairseq-train  examples/random_sampler/data-bin/$EXPERIMENT_NAME \
                                            ...
                                            --save-dir "de_en_experiments/${EXPERIMENT_NAME}" \
                                            --source-lang=$src \
                                            --target-lang=$tgt \
                                            --seed $SEED \
                                            ...
                                            --no-epoch-checkpoints > de_en_experiments/${EXPERIMENT_NAME}/$EXPERIMENT_NAME.log
                                            


CUDA_VISIBLE_DEVICES=$DEVICE nohup fairseq-generate examples/random_sampler/data-bin/$EXPERIMENT_NAME \
                                        --path de_en_experiments/${EXPERIMENT_NAME}/checkpoint_best.pt \
                                        --batch-size 128 \
                                        --beam 5 \
                                        --max-len-a 1.2 \
                                        --max-len-b 10 \
                                        --remove-bpe > de_en_experiments/${EXPERIMENT_NAME}/bleu_unprocessed.log



cd de_en_experiments/${EXPERIMENT_NAME}
